...
登入頁沒有公開的「建立帳號」連結：**3 秒內連續點 logo 5 次**才會出現註冊表單。
**第一位註冊的使用者會自動成為系統管理員**，擁有所有權限。
其他人註冊後是「待審核」，不能登入；系統管理員的首頁會跳出通知（也會出現在備忘錄），
到「👥 帳號審核與權限」核准並勾選權限後才能使用。
...
| 📦 商品與生產 | 產品、**配方與成本試算**、原料、庫存、生產履歷、**產品月盤點**、**原物料進貨／盤點**、**原物料漲幅表** | 配方以「基準重量 × 倍數」計算（可改倍數、目標總重、預設重量），可列印 / 下載 Excel 配方表；月盤點自動帶入製作、出貨，輸入實盤算出每月使用量；進貨記錄數量、金額、批號、製造／有效日期，可同時記入支出表並更新原料成本；漲幅表依每次進貨單價算出比上次、3 個月、1 年的漲跌 |
...
|---|---|---|
| 13 | 最高系統管理員 | 全部功能（老闆） |
| 3 | 系統管理 | 帳號審核與權限、員工及會員、登入紀錄、郵件寄送 |
| 20 | 行事曆 | 行事曆（查看、新增、修改） |
| 21 | 市集營運 | 現場點餐、市集報表、出攤紀錄、報名連結（可讀行事曆） |
| 22 | 商品與生產 | 產品、配方與成本、原料、庫存、生產履歷、月盤點、原物料進貨／漲幅 |
| 23 | 銷售與客戶 | 訂單、出貨、客戶／合作廠商（可讀產品） |
| 24 | 財務 | 帳務、支出、品牌攤提、首頁收支；可查看出攤與訂單 |
| 25 | AI 行銷 | AI 文案發想 |
| 12 | 刪除資料 | 刪除任何資料都需要這個權限（個人的備忘錄、AI 文案除外） |
| 16 | 唯讀 | 只能查看，不能新增、修改、刪除 |

- **沒有勾選的模組，資料在伺服器端就會被擋下**：例如沒有商品權限就讀不到產品，沒有銷售／財務權限就讀不到客戶。會員個資（Users）只有系統管理員讀得到。
- 沒有任何權限的帳號，只能用首頁、自己的備忘錄、帳號設定和 AI 助理。
- 在「👥 帳號審核與權限」用勾選表格設定每個帳號的權限、啟用／停用，停用後對方下一次操作就會被登出。
- 只有最高系統管理員（13）可以授予或移除「最高系統管理員」；任何人都不能移除自己的管理權限。
- **忘記密碼**：登入頁送出「重設申請」後，系統管理員會收到通知；確認是本人後，在「帳號審核與權限」按「產生臨時密碼」（只顯示一次，對方所有裝置會被登出）。對方用臨時密碼登入後必須先設定新密碼。系統管理員不能重設最高管理員的密碼。
